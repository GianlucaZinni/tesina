...
def populate_static_data():
    ...
    # Crear razas (como ya lo tenías pero ahora con tipos bien vinculados)
    razas = []
    ...
    for sexo in sexos:
        for especie in especies:
            nombres_estados = estados_por_sexo_especie.get(sexo.nombre, {}).get(especie.nombre, [])
    ...
    estados_db = EstadoReproductivo.query.all()
    sexos = Sexo.query.all()
